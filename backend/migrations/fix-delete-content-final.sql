-- Fix delete_content_cascade - only delete from tables that exist
...
BEGIN
    ...
    -- Delete molecule view records (if exists)
    DELETE IGNORE FROM molecule_views WHERE content_id = content_id_to_delete;
...
SELECT 'Stored procedure updated: delete_content_cascade (removed task_completions and user_claim_ratings)' AS status;
