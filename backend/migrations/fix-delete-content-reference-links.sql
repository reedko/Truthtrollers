-- Fix delete_content_cascade to handle reference_claim_task_links properly
-- The issue: claims being deleted might be referenced in reference_claim_task_links from OTHER content
...
BEGIN
    ...
    -- Step 6: Delete claims ONLY if they're not referenced by:
    --   a) any other content (content_claims)
    --   b) any reference_claim_task_links (as task_claim_id or reference_claim_id)
    ...
    AND claim_id NOT IN (
        SELECT DISTINCT task_claim_id FROM reference_claim_task_links WHERE task_claim_id IS NOT NULL
    )
    AND claim_id NOT IN (
        SELECT DISTINCT reference_claim_id FROM reference_claim_task_links WHERE reference_claim_id IS NOT NULL
    );
...
SELECT '✅ Stored procedure updated: delete_content_cascade' AS status;
SELECT 'Now properly handles reference_claim_task_links foreign key constraints' AS fix;
