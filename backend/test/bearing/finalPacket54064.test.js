...
test("54064 Thompson attribution evidence cannot satisfy the substantive target (not a support slot)", () => {
  const withThompson = [
    ...evidence,
    { id: "thompson", url: "https://example.org/thompson-statement", title: "Statement of William W. Thompson", quote: "William Thompson revealed that the CDC manipulated data linking the MMR vaccine to autism.", summary: "", stance: "support", bearingScore: 0.58, bearingType: "direct", claimComponentAddressed: "attribution", evidenceTargetId: 173, evidenceTargetType: "substantive", identityRole: "attribution_document" },
  ];
  const packet = buildEvidencePacket({ claim, evidence: withThompson, minBearing: 0.35, maxItems: 5 });
  const thompson = packet.items.find((i) => i.url.includes("thompson-statement"));
  if (thompson) {
    // On the substantive target it is attribution_only and cannot be substantive support.
    assert.equal(thompson.compatibilityLabel, "attribution_only");
    assert.notEqual(thompson.stance, "support");
  }
});

test("54064 CDC/DeStefano study material is preferred over repeated allegation material", () => {
  const packet = buildEvidencePacket({ claim, evidence, minBearing: 0.35, maxItems: 5 });
  const cdcRank = packet.items.findIndex((i) => i.url.includes("archive.cdc.gov"));
  const allegationRank = packet.items.findIndex((i) =>
    i.url.includes("globenewswire") || i.url.includes("vaccineimpact") || i.url.includes("healthimpactnews"));
  assert.ok(cdcRank !== -1, "CDC study page present");
  // If an allegation representative is present at all, the study page outranks it.
  if (allegationRank !== -1) assert.ok(cdcRank < allegationRank, "study identity preferred over allegation");
});

test("54064 FactCheck generic MMR/autism material cannot occupy the direct substantive refute slot", () => {
  const withFactCheck = [
    ...evidence,
    { id: "factcheck", url: "https://www.factcheck.org/2015/mmr-vaccine-autism/", title: "MMR Vaccine Does Not Cause Autism", quote: "Numerous studies have found no link between the MMR vaccine and autism.", summary: "", stance: "refute", bearingScore: 0.57, bearingType: "indirect", claimComponentAddressed: "object", evidenceTargetId: 173, evidenceTargetType: "substantive" },
  ];
  const packet = buildEvidencePacket({ claim, evidence: withFactCheck, minBearing: 0.35, maxItems: 5 });
  const fc = packet.items.find((i) => i.url.includes("factcheck.org"));
  if (fc) {
    assert.notEqual(fc.compatibilityLabel, "direct_substantive");
    assert.notEqual(fc.stance, "refute");
  }
});
