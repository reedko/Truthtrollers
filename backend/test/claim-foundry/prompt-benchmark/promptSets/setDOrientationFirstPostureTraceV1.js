...
import { orientationFirstPostureTraceSchemaForArticle }
...
each candidate, do not finalize claimText until the proposition, assertion source,
article role, thesis consequences, and article use have been resolved.
...
4. ifSupportedEffect and ifRefutedEffect
...
P → assertion source → article role → effect if supported → effect if refuted →
article use → diagnostic transform → final claim wording → grounding and scope.
...
${serializeStructuredArticle(structuralBlocks, sourceUnits)}`,
  responseSchema: orientationFirstPostureTraceSchemaForArticle(article) };
}

// Preserve the raw traced response as a benchmark artifact while adapting only
// the verified pre-host inventory to the live CF1 shape.
...
export const SET_D_ORIENTATION_FIRST_POSTURE_TRACE_V1 = Object.freeze({
  id: "set-d-orientation-posture-trace-v1", label: "hidden from reviewers",
  status: "experimental", call1Only: true,
  call1: { version: "orientation-posture-trace-call1-v1",
    schemaName: "cf1_semantic_inventory_order_trace_v1",
    schemaHash: "80e8b7ddc45c89c00750dfb330f25cd06aa9d9038dde0266f6596698af0933b8",
