const strings = (maxItems = 20, maxLength = 500) => ({
  type: "array", maxItems, items: { type: "string", minLength: 1, maxLength },
});

// Relation-trace probe: preserve the supplied proposition and record the article's
// treatment separately, before asking the model to classify posture.
export const CF1_SOURCE_PROPOSITION_RESPONSE_SCHEMA_V1 = Object.freeze({
  name: "cf1_semantic_inventory_source_proposition_response_v1", strict: true,
  schema: {
    type: "object", additionalProperties: false,
    required: ["theme", "thesis", "pillars", "thesisHinge", "candidateClaims"],
    properties: {
      theme: { type: "object", additionalProperties: false, required: ["text", "sourceUnitIds"],
        properties: { text: { type: "string", minLength: 1, maxLength: 500 }, sourceUnitIds: strings(12, 20) } },
      thesis: { type: "object", additionalProperties: false, required: ["text", "sourceUnitIds"],
        properties: { text: { type: "string", minLength: 1, maxLength: 500 }, sourceUnitIds: strings(12, 20) } },
      pillars: { type: "array", minItems: 1, maxItems: 8, items: {
        type: "object", additionalProperties: false,
        required: ["label", "text", "importance", "sourceUnitIds"],
        properties: { label: { type: "string", minLength: 1, maxLength: 140 },
          text: { type: "string", minLength: 1, maxLength: 500 },
          importance: { type: "string", enum: ["load_bearing", "major", "supporting"] },
          sourceUnitIds: strings(12, 20) },
      } },
      thesisHinge: { type: "string", enum: ["substance", "attribution", "mixed"] },
      candidateClaims: { type: "array", minItems: 1, maxItems: 15, items: {
        type: "object", additionalProperties: false,
        required: ["sourceProposition", "assertionSource", "sourceUnitIds", "articleResponse",
          "articleResponseUnitIds", "ifSupportedEffect", "ifRefutedEffect", "articleUse",
          "articleRole", "scoreTransformCheck", "materiality", "relatedPillarLabels", "scope",
          "evidenceUsefulnessHint"],
        properties: {
          sourceProposition: { type: "string", minLength: 1, maxLength: 500 },
          assertionSource: { type: "string", minLength: 1, maxLength: 300 },
          sourceUnitIds: strings(12, 20),
          articleResponse: { type: "string", enum: ["none", "supports", "challenges", "rejects", "qualifies", "defers"] },
          articleResponseUnitIds: { type: "array", maxItems: 12, items: { type: "string", minLength: 1, maxLength: 20 } },
          ifSupportedEffect: { type: "string", enum: ["strengthens", "weakens", "unchanged", "unclear"] },
          ifRefutedEffect: { type: "string", enum: ["strengthens", "weakens", "unchanged", "unclear"] },
          articleUse: { type: "string", enum: ["endorsed", "opponent_to_rebut", "rejected", "reported", "background", "qualification", "unclear"] },
          articleRole: { type: "string", enum: ["thesis", "pillar", "pillar_support", "opponent_claim", "qualification", "consistency_hinge"] },
          scoreTransformCheck: { type: "string", enum: ["normal", "invert", "none", "unresolved"] },
          materiality: { type: "string", enum: ["high", "medium", "low"] },
          relatedPillarLabels: strings(4, 140),
          scope: { type: "string", minLength: 1, maxLength: 400 },
          evidenceUsefulnessHint: { type: "string", minLength: 1, maxLength: 240 },
        },
      } },
    },
  },
});

export function sourcePropositionResponseSchemaForArticle(article) {
  const schema = structuredClone(CF1_SOURCE_PROPOSITION_RESPONSE_SCHEMA_V1);
  if (String(article?.text ?? "").length >= 5_000) schema.schema.properties.candidateClaims.minItems = 8;
  return schema;
}

export const CF1_SOURCE_PROPOSITION_RESPONSE_SCHEMA_V2 = Object.freeze((() => {
  const schema = structuredClone(CF1_SOURCE_PROPOSITION_RESPONSE_SCHEMA_V1);
  schema.name = "cf1_semantic_inventory_source_proposition_response_v2";
  schema.schema.properties.candidateClaims.maxItems = 12;
  return schema;
})());

export function sourcePropositionResponseSchemaV2ForArticle(article) {
  const schema = structuredClone(CF1_SOURCE_PROPOSITION_RESPONSE_SCHEMA_V2);
  if (String(article?.text ?? "").length >= 5_000) schema.schema.properties.candidateClaims.minItems = 8;
  return schema;
}
