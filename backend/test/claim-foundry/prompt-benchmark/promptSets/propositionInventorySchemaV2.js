...
// Test-only B/C repair schema. Property order is intentional because Structured
// Outputs emits keys in schema order. It makes the dependency chain observable
// instead of asking the model to finalize the claim before source and posture.
export const CF1_SEMANTIC_INVENTORY_PROPOSITION_V2 = Object.freeze({
  name: "cf1_semantic_inventory_proposition_v2", strict: true,
  schema: {
...
export function propositionInventorySchemaForArticle(article) {
  const responseSchema = structuredClone(CF1_SEMANTIC_INVENTORY_PROPOSITION_V2);
  if (String(article?.text ?? "").length >= 5_000) {
    responseSchema.schema.properties.candidateClaims.minItems = 8;
  }
