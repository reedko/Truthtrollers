#!/usr/bin/env node
// CF5 PromptCF5-v0003/v0004 blind regression runner. v0001 data reused from
// cf5-promptcf5-v002's v0001_repeat1 runs (verified byte-identical to current
// production prompts.js earlier this cycle, unchanged since). v0003 ("Basis") and
// v0004 ("Evidence Questions") run fresh: F02/F03/F06 x 1 repeat each = 6 generation
// calls. No changes to model/settings/schema/preprocessing/validation/repair — reuses
...
import { buildPromptCF5v003, PROMPT_CF5_V003_VERSION } from "./promptCF5v003.js";
import { buildPromptCF5v004, PROMPT_CF5_V004_VERSION } from "./promptCF5v004.js";
...
const promptV0003Hash = hash(readFileSync(path.join(outDir, "prompt_v0003.txt"), "utf8"));
const promptV0004Hash = hash(readFileSync(path.join(outDir, "prompt_v0004.txt"), "utf8"));
...
const versionBuilders = {
  "PromptCF5-v0003": buildPromptCF5v003,
  "PromptCF5-v0004": buildPromptCF5v004,
};
...
for (const fixture of fixtures) {
  ...
  const v0001Manifest = {
    schemaVersion: "cf5.promptV003V004RunManifest.v1",
  ...
  // --- v0003, v0004: fresh, 1 repeat each ---
  for (const [version, builder] of Object.entries(versionBuilders)) {
    const promptHash = version === PROMPT_CF5_V003_VERSION ? promptV0003Hash : promptV0004Hash;
    const prompt = builder({ article, units });
    const versionDestDir = path.join(fixtureDir, `${version.replace("PromptCF5-", "").toLowerCase()}_repeat1`);
    ...
    const manifest = {
      schemaVersion: "cf5.promptV003V004RunManifest.v1",
      fixtureId: fixture, promptVersion: version, repeat: 1,
      reusedFrom: null,
      requestedModel: model, returnedModel: genRes.model,
      promptHash, schemaHash, gitCommit,
    ...
    runIndex.push({ fixture, promptVersion: version, repeat: 1, dir: versionDestDir, manifest, finalClaims: result.finalClaims });
    console.log(`  ${fixture} ${version} repeat 1: ${result.finalClaims.length} claims`
      + (result.repairResult ? " (repaired)" : ""));
  }
}

writeFileSync(path.join(outDir, "run_config.json"), JSON.stringify({
  schemaVersion: "cf5.promptV003V004RunConfig.v1",
  objective: "Evaluate two alternative prompt ontologies against PromptCF5-v0001: "
    + "v0003 frames ClaimFoundry as discovering a minimal evidentiary basis of 'basis "
    + "elements'; v0004 frames it as discovering independent 'evidence questions'. "
    + "Tests whether ontology framing (not just word-level substitution) affects "
    + "decomposition quality.",
  fixtures, model, reasoningEffort: "none", timeoutMs,
  repeatsPerFixturePerVersion: 1,
  totalGenerationRuns: fixtures.length * 3,
  v0001Routing: "reused from cf5-promptcf5-v002's v0001_repeat1 runs",
  v0003Routing: "run fresh", v0004Routing: "run fresh",
  promptV0001Hash, promptV0003Hash, promptV0004Hash, schemaHash, gitCommit,
...
console.log(`\nPromptCF5-v0003/v0004 blind regression complete → ${outDir}`);
