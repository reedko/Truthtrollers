#!/usr/bin/env node
// CF5 PromptCF5-v0007 blind regression runner. v0001 data reused from
// cf5-promptcf5-v002's v0001_repeat1 run (same source already reused unmodified for
// the v0003/v0004, v0005, and v0006 experiments). v0007 ("MISSION/TASK/OUTPUT hybrid,
// single-predication worked examples + thematic minimum-coverage framing") runs fresh:
// F02/F03/F06 x 1 repeat = 3 generation calls, per the user's explicit request to run
// this once. No changes to model/settings/schema/preprocessing/validation/repair —
// reuses pipeline.js/validation.js/schemas.js exactly as-is.
...
const promptV0007Hash = hash(readFileSync(path.join(outDir, "prompt_v0007.txt"), "utf8"));
...
for (const fixture of fixtures) {
  ...
  const v0001Manifest = {
    schemaVersion: "cf5.promptV007RunManifest.v1",
  ...
  // --- v0007: fresh, 1 repeat ---
  const versionDestDir = path.join(fixtureDir, "v0007_repeat1");
  ...
  const manifest = {
    schemaVersion: "cf5.promptV007RunManifest.v1",
    fixtureId: fixture, promptVersion: PROMPT_CF5_V007_VERSION, repeat: 1,
    reusedFrom: null,
  ...
  runIndex.push({ fixture, promptVersion: PROMPT_CF5_V007_VERSION, repeat: 1, dir: versionDestDir, manifest, finalClaims: result.finalClaims });
  console.log(`  ${fixture} ${PROMPT_CF5_V007_VERSION} repeat 1: ${result.finalClaims.length} claims`
    + (result.repairResult ? " (repaired)" : ""));
}

writeFileSync(path.join(outDir, "run_config.json"), JSON.stringify({
  schemaVersion: "cf5.promptV007RunConfig.v1",
  objective: "Evaluate PromptCF5-v0007 against PromptCF5-v0001. v0007 combines v0005's "
    + "single-predication split-on-different-verdicts language (now with worked "
    + "GOOD/BAD examples) with v0006's 'minimum number of representative "
    + "single-predication assertions per theme' coverage framing, under an explicit "
    + "MISSION / TASK / OUTPUT section structure not used by any prior version.",
  fixtures, model, reasoningEffort: "none", timeoutMs,
  repeatsPerFixturePerVersion: 1,
  totalGenerationRuns: fixtures.length * 2,
  v0001Routing: "reused from cf5-promptcf5-v002's v0001_repeat1 runs",
  v0007Routing: "run fresh",
...
console.log(`\nPromptCF5-v0007 blind regression complete → ${outDir}`);
