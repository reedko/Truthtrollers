# Call 1A prompt — full text for chat-window testing (2026-07-19)

Verbatim from the CURRENT live `splitCall1aDiscoveryPromptV1.js` (the version with ceiling/floor
already removed — there is NO claim-count/budget line, so a chat test shows the model's natural
count). The live prompt is NOT edited. Fill the `<...>` placeholders with your article.

The real call sends `system` + `user` as two messages plus a structured-output schema. For a chat
window: paste SYSTEM first, then USER, then the OUTPUT FORMAT block at the very bottom (the chat
model needs it because there's no API schema enforcing the shape).
...
STRUCTURED ARTICLE:
<STRUCTURED_ARTICLE — paste the serialized article here: the structural blocks and their U#### source
units, in source order, exactly as the pipeline serializes them>
...
═══════════════════════════════════════════════════════════════════════════════
OUTPUT FORMAT  (sent as a structured-output schema via the API — paste this when testing in a
chat window so the model returns the right shape; no min/max count is imposed)
═══════════════════════════════════════════════════════════════════════════════

Return ONLY a JSON object of this shape:

{
  "theme":     { "text": "<full-sentence proposition>", "sourceUnitIds": ["U####", ...] },
  "thesis":    { "text": "<full-sentence proposition>", "sourceUnitIds": ["U####", ...] },
  "thesisHinge": "attribution" | "substance" | "mixed",
  "pillars": [
    { "label": "<short label>", "text": "<proposition>", "importance": "load_bearing" | "major" | "minor", "sourceUnitIds": ["U####", ...] }
  ],
  "candidateClaims": [
    {
      "claimText": "<one complete atomic proposition, in the asserter's original polarity>",
      "sourceUnitIds": ["U####", ...],
      "materiality": "high" | "medium" | "low",
      "relatedPillarLabels": ["<exact pillar label>", ...],
      "scope": "<short scope, e.g. products / regulation / national>",
      "evidenceUsefulnessHint": "<one sentence on what external evidence could test it>"
    }
  ]
}

Notes for the chat test:
- 1A candidate claims carry NO posture/source fields (no articleUse, articleRole, assertionSource,
  effects) — those are decided later by stage 1B / the host. Do not add them.
- There is intentionally no claim-count floor or ceiling in this prompt, so the number of
  candidateClaims is whatever the model naturally produces.
