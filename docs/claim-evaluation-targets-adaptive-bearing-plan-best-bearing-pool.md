...
RETRIEVAL_STRATEGY=best_bearing_pool
MIN_HIGH_BEARING_CLAIMS_PER_TARGET=5
...
SEARCH_PROVIDER_BUDGET_PER_TARGET_USD=0.05
SEARCH_MAX_PROVIDERS_PER_TARGET=7
SEARCH_MAX_SOURCES_TO_SCRAPE_PER_TARGET=10
...
Add explicit retrieval strategy configuration:
...
```text
cost_saver:
  Call providers adaptively.
  Stop provider expansion once the target has enough high-bearing assertions.

best_bearing_pool:
  Default mode for this plan.
  Call all enabled providers for the target/query pack first, within configured budget.
  Normalize and dedupe all returned candidates.
  Pre-score snippets across the merged candidate pool.
  Select the highest-bearing candidate sources for scraping.
  Continue source processing until five unique high-bearing assertions are found or the source ceiling is exhausted.

diagnostic_bakeoff:
  Call all selected providers.
  Preserve provider-specific yield data.
  Optionally continue beyond threshold to compare provider value.
```

Clarify provider search versus source processing:

```text
Provider search:
  In best_bearing_pool mode, run all enabled providers up front for the evaluation target, within budget.

Source processing:
  Do not scrape every returned source.
  Merge, dedupe, and snippet-score candidates first.
  Scrape/process only the strongest candidate sources needed to reach the high-bearing assertion threshold.
```
...
This is intentionally different from `cost_saver` mode. Cost-saver mode remains available, but it is not the default for this plan.
...
```text
Generate target-specific query pack
        |
Run enabled providers according to retrieval strategy
        |
Normalize and dedupe all provider results
        |
Pre-score snippets for bearing against the evaluation target
        |
Rank merged candidate pool by:
  - snippet bearing
  - provider rank
  - source type / primary-source likelihood
  - exact entity or study match
  - provider diversity
  - freshness/date where relevant
        |
Scrape and fully process the highest-ranked source
        |
Extract source assertions
        |
Post-scrape bearing-score assertions against the evaluation target
        |
Persist all qualifying unique high-bearing assertions
        |
Accumulated count >= 5?
    | yes
    Finish active source, then stop before next source
    | no
    Process next source from ranked merged pool
        |
If merged pool is exhausted below five:
    optionally expand query pack or mark unresolved
```
...
- Queries generated
- Sources processed
- Unique high-bearing assertion count, shown as `n / 5`
...
- [ ] Do not claim completion merely because five claims were encountered midway through an active source.
...
- [ ] Retrieval expands when fewer than five high-bearing claims are found.
...
- [ ] Best-bearing-pool mode searches all enabled configured providers before source processing.
- [ ] Results from all enabled providers are merged and deduped before candidate selection.
- [ ] The system aims for five unique high-bearing assertions per evaluation target.
- [ ] The system stops only after finishing the active source once five unique high-bearing assertions are reached.
- [ ] Unprocessed search results are not linked as evidence.
- [ ] Merely topical assertions do not count toward the five.
- [ ] Provider provenance is preserved for each candidate and each linked assertion.
- [ ] Cost-saver mode remains available but is not the default.
...
- Retrieval expands automatically when the first sources yield fewer than five unique high-bearing claims.
