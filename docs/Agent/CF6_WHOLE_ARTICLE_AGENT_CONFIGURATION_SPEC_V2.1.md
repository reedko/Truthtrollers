...
The agent may instead inspect after multiple updates, revise theses late, request inspection more than once, finalize after one inspection, or abstain. The host must not select the next semantic action. For the first experiment, illegal actions are rejected by the tool implementation with a typed error rather than hidden via schema changes — schema hiding breaks the prompt-cache prefix (see §9.5.2).
...
### 9.5 Cache-optimization contract (getting close to "reading is free")

"Free" stays banned. But the cached-input coefficient is controllable, and this contract
drives it toward its floor (~10% of full price per re-included prefix token on current
top-tier models). The article then bills roughly once at full price plus ~10% per
subsequent turn — about 1.3 articles total for a 4-turn loop, versus the manager's ~6.8.

Mechanics (all mandatory for the first experiment):

1. **Prefix ordering.** Prompt caching matches on exact byte-identical prefixes from the
   front. Assemble every request as: instructions → tool schemas → article → typed
   workbench (deltas last). Stable content first, byte-identical every turn.

2. **Byte-stable tool exposure.** Expose all tools every turn with byte-identical
   serialized schemas. Conditional/state-dependent exposure changes the schema block,
   which sits in the prefix — cache miss, article re-billed at full price. With the
   three-tool surface the always-on schema cost is small and cached after turn 1;
   the cache invalidation it avoids is far larger. Legal-state enforcement moves to the
   tool implementations (reject an illegal call with a typed error) instead of schema
   hiding. State-dependent exposure may return in a later experiment only with measured
   evidence it pays for its cache misses.

3. **No dynamic tokens in the stable region.** No timestamps, turn counters, run
   clocks, or per-turn IDs in instructions or tool descriptions. Anything per-turn goes
   in the trailing workbench.

4. **Compact mirror results.** Tool results become part of the next turn's prefix.
   `inspect_working_package` returns IDs and set-diffs, never full package echoes.

5. **TTL awareness.** Provider caches live minutes (5–10 typical; longer paid tiers
   exist). Run the loop's turns back-to-back. A mid-run human pause loses the cache and
   re-pays the article once — acceptable, but record it.

6. **Cached-discount rate is a model-selection criterion.** A provider with 90%-off
   cached input halves-to-quarters the loop's article cost versus a 50%-off provider.

Measured targets (reported, enforced by the fail-fast boundary):

- **Article cost multiplier** := total billed input token-equivalents ÷ article tokens.
  First-run target ≤ 1.5×. (Manager baseline: ~6.8×.)
- **Cache-hit assertion:** from request 2 onward, ≥ 90% of input tokens must report as
  cached, or hard-abort before the next billable request — a mid-loop cache miss means
  the prefix drifted (mutated instruction, reordered schema) and is a wiring bug to fix
  on turn 2, not discover in the bill.
