...
Date: 2026-07-28 (R1 reconciliation)  
Status: Implementation plan, reconciled to the governing spec  
Authority: **`CF6 Whole-Article Agent Configuration Spec v2.1` GOVERNS.** Where this plan
and v2.1 conflict, v2.1 wins. Known reconciliations applied in R1:
(a) tool surface is v2.1's three tools + terminal (`update_working_package`,
`inspect_working_package`, `finalize_working_package`, `abstain_or_request_review`) —
this plan's five contracts in §4 are implemented inside that surface per the mapping
note at §4;
(b) tool schemas are **byte-stable across all turns** (v2.1 §9.5.2) — the
state-dependent exposure table in §5 is superseded; legality is enforced inside tool
implementations via typed errors;
(c) the F03 acceptance items in §9 are **evaluator-only** (v2.1 §10) and must never be
runtime-visible;
(d) token accounting adds the article-cost-multiplier ≤1.5× target and the ≥90%
cached-input assertion from request 2 (v2.1 §9.5).  
...
6. tool schemas are byte-identical across all turns (cache-prefix stability, v2.1 §9.5); and
...
> **R1 mapping (v2.1 governs the tool surface):** the five contracts below define the
> *behaviors*, but they are implemented as v2.1's three tools + terminal:
> - §4.1 `identify_theses` → the `setTheses[]` field of `update_working_package`
>   (theses are model-authored working-package state; no separate forced turn);
> - §4.2 `propose_claims` → the `upsertClaims[]` / `removeClaimIds[]` fields of
>   `update_working_package`;
> - §4.3 `validate` + §4.4 `check_coverage` → merged into one
>   `inspect_working_package` mirror (one report, structural validation + exact
>   coverage accounting, so the host cannot masquerade as two semantic judges);
> - §4.5 `finalize` → `finalize_working_package`, gates unchanged;
> - plus v2.1 §6.4 `abstain_or_request_review` as the typed terminal alternative.
> All host-behavior requirements below (authorization, atomicity, receipts, no
> semantic judgment) apply unchanged to the merged implementations.
...
## 5. Tool legality enforcement (R1: byte-stable exposure, superseding the state table)

**Superseded:** the original state-dependent exposure table is retired for the first
experiment. Changing the exposed tool list between turns changes the serialized schema
block, which sits in the prompt-cache prefix — every change is a cache miss that
re-bills the article at full price (v2.1 §9.5.2).

R1 rule:

- All tools (`update_working_package`, `inspect_working_package`,
  `finalize_working_package`, `abstain_or_request_review`) are exposed on **every**
  turn with **byte-identical** serialized schemas.
- Legality is enforced **inside** each tool implementation: an action illegal in the
  current state returns a compact typed error (e.g. `ILLEGAL_IN_STATE: finalize
  requires a non-stale inspection`), which the agent sees and acts on. The finalize
  gates of §4.5 are unchanged — they move from schema-hiding to in-tool rejection.
- Multiple legal actions remain available wherever the state permits them. The runner
  still does not hard-code any sequence.
- State-dependent exposure may return in a later experiment only with measured evidence
  that its benefit exceeds its cache-miss cost.
...
R1 additions (v2.1 §9.5):

- report the **article cost multiplier** (total billed input token-equivalents ÷ article
  tokens); first-run target ≤ 1.5× (manager baseline ~6.8×);
- enforce the **cache-hit assertion**: from request 2 onward, ≥ 90% of input tokens
  report as cached, or hard-abort before the next billable request — a mid-loop cache
  miss is a prefix-drift wiring bug, caught on turn 2, not in the bill.
...
- tool schemas are byte-identical across all turns;
- illegal-in-state calls are rejected in-tool with typed errors;
...
> **Evaluator-only (v2.1 §10):** every item below is scored by the post-run evaluation
> harness / human review. None of these expectations — thesis names, Thompson, crux,
> unit ranges — may appear in agent instructions, tool schemas, mirror logic, host
> diagnostics, or any model-visible runtime artifact. A fixture-aware assertion inside
> the runtime is a contamination violation and invalidates the run.
...
- tool schemas are not byte-identical across turns (cache-prefix instability);
- the ≥90% cached-input assertion (v2.1 §9.5) is not wired into the fail-fast boundary;
