heya! big CFX revamp in progress. I think codex has already overwritten a lot of code, but wrerdly it doesn't seem aware of the old pipeline, process. So can you generate a doc for me, and there are some already on file that may actaully be pretty up to date even thought they are 2 months old, that just goes thru the process of the evidence engine, the legacty we've been trying to replace, and step trhu how it works bwhat arre the model calls, u probabaly older backend logs could help with this too, of importance, we would scrape the evidence doc, and then match the case calims with the srouce claims somehow, and I want to see how we did that, how many calles, etc, and we also generate a source crest trating, I wanted to see how many calls, etc. so this would be from before TM4 because at that point codex and you kind of broke everything, but the prod pipeline might still have been there? anyway reach back and give a nice step by step in a .md file for me
