...
import { fetchTaskClaimsWithRatings } from "../../services/useDashboardAPI";
...
interface ClaimItem {
  claim_id: number;
  claim_text: string;
  veracity: number | null; // from -1 to 1, used to determine 'true' or 'false'
}
...
const TrueFalseQuizGame: React.FC = () => {
  ...
  useEffect(() => {
    const loadClaims = async () => {
      ...
      const result = await fetchTaskClaimsWithRatings(
  ...
  const handleSubmit = () => {
    ...
    claims.forEach((claim) => {
      const predictedTrue = claim.veracity !== null && claim.veracity > 0;
