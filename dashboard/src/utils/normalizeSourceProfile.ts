...
const SOURCE_TYPE_PATTERNS: [RegExp, SourceType][] = [
  [
    /\.gov\b|government|official|federal|state\.(us|gov)|white\s?house|congress/i,
    "government",
  ],
  [
    /\.edu\b|university|univers|college|journal\b|academic|research|science|scholar|pubmed|arxiv|preprint/i,
    "academic",
  ],
  [
    /wikipedia|britannica|encyclopedia|merriam|dictionary|reference|library/i,
    "reference",
  ],
  [
    /advocacy|think.?tank|institute|foundation|policy\s?org|ngo|nonprofit|501c/i,
    "advocacy",
  ],
  [
    /corp|inc\b|ltd\b|llc\b|corporate|business|pr\b|press.?release|investor|sec\.gov/i,
    "corporate",
  ],
  [/opinion|editorial|commentary|column|op.?ed|blog\b/i, "opinion"],
  [
    /twitter|x\.com|facebook|reddit|instagram|tiktok|social|forum|user.?generat/i,
    "social",
  ],
  [
    /news|press|times|herald|post|tribune|gazette|media|reporter|journalist|wire|associated\s?press|reuters|ap\b|bbc|cnn|nbc|abc|cbs|npr/i,
    "journalism",
  ],
];
...
function detectSourceType(data: RawSourceData): SourceType {
  ...
  const candidate = [data.publisher_name, data.source_name, data.media_source]
    .filter(Boolean)
    .join(" ");
