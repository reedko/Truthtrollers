...
export class EvidenceEngine {
  ...
  async retrieveCandidates(claim, queries, opt) {
    ...
    // Group deduplicated candidates by PURPOSE LANE, take top N per bucket.
    // This guarantees *purpose* diversity (distinct evidentiary jobs) instead of
    // the old stance-quota bucketing. Stance is never used here; it is classified
    // downstream from the returned content.
    const topKPerLane = opt.topKPerIntent ?? opt.topKPerLane ?? 4;
    ...
    let finalCandidates = [];
    for (const [lane, bucket] of Object.entries(byLane)) {
      // Background/context lanes contribute fewer candidates than substantive ones.
      const limit = lane === "causal_background" || lane === "source_context"
        ? Math.max(2, Math.ceil(topKPerLane / 2))
        : topKPerLane;
      const sorted = bucket.sort((a, b) => (b.score ?? 0) - (a.score ?? 0));
      finalCandidates.push(...sorted.slice(0, limit));
      logger.log(`🎯 [EV][purpose-lane][${claim.id}] ${lane}: ${Math.min(sorted.length, limit)}/${sorted.length} selected`);
    }
