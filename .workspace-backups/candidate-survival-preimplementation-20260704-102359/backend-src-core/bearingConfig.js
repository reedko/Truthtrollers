...
export function normalizeBearingGatingConfig(raw = {}, env = process.env) {
  ...
  return {
    ...
    minDeliveredSourcesPerContent: clampInteger(raw.minDeliveredSourcesPerContent, defaults.minDeliveredSourcesPerContent, 1, 100),
    maxSourceAttemptsPerContent: clampInteger(raw.maxSourceAttemptsPerContent, defaults.maxSourceAttemptsPerContent, 1, 200),
    maxSourcesComparedPerClaim: clampInteger(raw.maxSourcesComparedPerClaim, defaults.maxSourcesComparedPerClaim, 3, 50),
    minBearingLinksPerClaim: clampInteger(raw.minBearingLinksPerClaim, defaults.minBearingLinksPerClaim, 1, 20),
    queryLanesPerStance: clampInteger(raw.queryLanesPerStance, defaults.queryLanesPerStance, 1, 6),
    ...
    maxIdentitySlotsPerClaim: clampInteger(raw.maxIdentitySlotsPerClaim, defaults.maxIdentitySlotsPerClaim, 0, 3),
    maxSteelmanSlotsPerClaim: clampInteger(raw.maxSteelmanSlotsPerClaim, defaults.maxSteelmanSlotsPerClaim, 0, 2),
    minHighBearingClaimsPerTarget: clampInteger(raw.minHighBearingClaimsPerTarget, defaults.minHighBearingClaimsPerTarget, 1, 20),
    finishActiveSourceOnThreshold: raw.finishActiveSourceOnThreshold !== false,
    perClaimLimits,
  };
}
