...
export class EvidenceEngine {
  ...
  async generateQueries(claim, ctx, n = 6, searchMode = null) {
    ...
    if (targetAware) {
      ...
      fallbackUser = `Claim: {{claimText}}\nContext: {{context}}\nEvidence need: {{evidenceNeed}}\n\nProduce at most {{n}} precise queries for the listed evidence targets; when n is 9, produce exactly three support, three refute, and three nuance/context queries. Every result must include evidenceTargetId (the supplied lane id), evidenceTargetType, stanceGoal, and bearingRequirement. Preserve exact numbers, populations, dates, doses, and timeframes in at least one query when present. Use target-appropriate retrieval language such as registry, cohort, dataset, transcript, original study, systematic review, methodology, or limitations only when it fits the evidence target. Do not broaden the claim.`;
...
IMPORTANT: Design your queries to actively seek out sources with different perspectives. For refute queries, look for credible counterarguments, debunking sites, fact-checks, or alternative evidence. For support queries, look for sources that would confirm or provide evidence for the claim. For nuance queries, look for sources that provide context, caveats, or partial support/refutation.`;
    ...
    try {
      ...
      // Continue through the deterministic completion path. In live bearing
      // mode this still guarantees the configured 3/3/3 query lanes.
      out = { queries: [] };
    }
    ...
    for (const c of chunks) {
      ...
      if (!prev || (c.score ?? 0) > (prev.score ?? 0)) {
        best.set(id, {
          ...c,
          targetProvenance: [
            ...(prev?.targetProvenance || []),
            occurrence,
          ],
        });
      } else {
        prev.targetProvenance = [...(prev.targetProvenance || []), occurrence];
      }
    }
