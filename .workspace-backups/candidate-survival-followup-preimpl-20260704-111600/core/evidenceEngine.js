...
export class EvidenceEngine {
  ...
  async retrieveCandidates(claim, queries, opt) {
    ...
    for (const { candidate, reason } of dropped) {
      logCandidateDrop({ claim, taskContentId: opt.taskContentId || null, stage: DROP_STAGES.POOL, candidate, reason });
    }
      ...
      if (opt.enableBearingGating) {
        ...
        for (const candidate of excludedFromLlm) {
          logCandidateDrop({
            claim,
            taskContentId: opt.taskContentId || null,
            stage: DROP_STAGES.LLM_BEARING,
            candidate,
            reason: "beyond_llm_bearing_cap_survives_with_deterministic_score",
          });
        }
        candidates = ordered;
      }
