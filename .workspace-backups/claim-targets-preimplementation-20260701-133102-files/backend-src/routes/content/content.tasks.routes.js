...
router.get("/api/user-tasks/:user_id", async (req, res) => {
 ...
 (
    ...
    WHERE cu.user_id = ?
      AND EXISTS (SELECT 1 FROM content_relations cr WHERE cr.content_id = t.content_id)
...
router.get("/api/search-tasks", async (req, res) => {
  ...
  const sql = includeInactive ? `
    ...
    FROM content t
    WHERE EXISTS (SELECT 1 FROM content_relations cr WHERE cr.content_id = t.content_id)
      AND (
        t.content_name LIKE ?
...
router.post("/api/submit-text", async (req, res) => {
  ...
  try {
    ...
    await linkPublisherRole(query, taskContentId, {
      publisherId,
      role: "primary_source",
      isPrimary: true,
      method: "legacy_task_route",
    });
