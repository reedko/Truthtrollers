...
export async function bingSearch({ query, topK = 5, prefer = [], avoid = [] }) {
  ...
  try {
    ...
    const bearingShadowEnabled = process.env.ENABLE_BEARING_GATING === "true" ||
      process.env.ENABLE_BEARING_SHADOW === "true" ||
      (process.env.ENABLE_BEARING_SHADOW !== "false" && process.env.NODE_ENV !== "production");
    ...
    const mapped = items.map((item, idx) => ({
      ...
      ...(bearingShadowEnabled ? {
        provider: "bing",
        providerRank: idx + 1,
        providerScore: typeof item.rank === "number" ? item.rank : 1 / (idx + 1),
      } : {}),
    }));
