...
import { extractAuthors, followProfileLinks, mergeAuthors } from "../utils/extractAuthors.js";
...
async function ensureReferencePublisherLink(query, { referenceContentId, url, publisher }) {
  ...
  const role = isSocialPublisher
    ? "distribution_channel"
    : isPublisherProxy ? "publication_venue" : "primary_source";
  await linkPublisherRole(query, referenceContentId, {
    publisherId,
    role,
    isPrimary: true,
    confidence: publisher?.confidence === "proxy" ? 0.6 : 0.75,
    method: publisher?.confidence === "extension_metadata" ? "extension_metadata" : "legacy_adapter",
    replaceRole: true,
  });
...
export async function resolvePublisherChain(url, depth, dbQuery) {
  ...
  try {
    ...
    let pub = (await processPublishingIdentity({ $, sourceUrl: url })).legacyPublisher;
...
export async function scrapeReference(query, {
  ...
  publishingIdentity: providedPublishingIdentity,
  ...
  try {
    ...
    const hasProvidedAuthors = Array.isArray(providedAuthors) && providedAuthors.length > 0;
    ...
    // here so that block isn't fighting the canonical identity pipeline.
    const initialIdentityResult = providedPublishingIdentity
      ? await processPublishingIdentity({ identity: providedPublishingIdentity })
      : $ && !botBlocked && !isFacebookUrl
        ? await processPublishingIdentity({ $, sourceUrl: url })
        : !$ && raw_text
          ? await processPublishingIdentity({ documentType: "pdf", pdfText: raw_text, sourceUrl: url })
          : { identity: null, legacyPublisher: null };
    let publishingIdentity = initialIdentityResult.identity;
    // Extension/PDF metadata often contains only the first author. Merge it
    // with the canonical document extractor instead of allowing a partial
    ...
    if (!publishingIdentity && platform === "facebook") {
      publishingIdentity = (await processPublishingIdentity({
        sourceUrl: url,
        socialContext: {
          platform,
          distributionChannel: distribution_channel,
          linkedUrl: linked_url,
          linkedPublisher: linked_publisher,
          provenance: socialProvenance,
        },
      })).identity;
    }
    ...
    logger.log(
      `🧪 [scrapeReference] Metadata audit: title="${finalTitle.slice(0, 100)}" ` +
      `authors=${authors.length}${authors.length ? ` [${authors.map((author) => author?.name || author).join(", ")}]` : " (none explicitly identified)"} ` +
      `publisher="${publisher?.name || "none"}" thumbnail=${thumbnail ? "yes" : "no"} ` +
      `botBlocked=${botBlocked} html=${raw_html ? "yes" : "no"}`
    );
    ...
    if (platform === "facebook" && linked_url && linked_url !== url) {
      try {
        const linkedName = socialProvenance?.substantiveSourcePublisher || linked_publisher || null;
        linkedContentId = await createContentInternal(query, {
          content_name: socialProvenance?.sharedSourceTitle || linked_url,
          url: linked_url,
          media_source: linkedName || "Unknown",
          topic: "AI Evidence",
          subtopics: [],
          content_type: "reference",
          taskContentId: referenceContentId,
          details: `Linked source shared by social content ${referenceContentId}`,
          platform: "web",
        });
        const linkedIdentity = {
          version: SOURCE_IDENTITY_VERSION,
          source_url: linked_url,
          document: { article_type: null, publication_year: null, volume: null, issue: null, identifiers: [] },
          entities: {
            publishing_organization: {
              name: linkedName,
              entity_type: "organization",
              method: socialProvenance?.substantiveSourcePublisher ? "linked_source_resolution" : "linked_domain",
              confidence: socialProvenance?.substantiveSourcePublisher ? 0.75 : 0.35,
              evidence: linked_publisher || linked_url,
            },
          ...
          context: {
            context_type: "web",
            platform: "web",
            publisher_name_observed: linkedName,
            extraction_method: socialProvenance?.substantiveSourcePublisher ? "linked_source_resolution" : "linked_domain",
            extraction_confidence: socialProvenance?.substantiveSourcePublisher ? 0.75 : 0.35,
            extractor_version: SOURCE_IDENTITY_VERSION,
          },
          candidates: [],
          warnings: linkedName ? [] : ["Linked source publisher was not resolved."],
        };
        await processPublishingIdentity({ query, contentId: linkedContentId, identity: linkedIdentity });
      } catch (linkedError) {
        logger.warn(`⚠️  [scrapeReference] Linked social source persistence skipped: ${linkedError.message}`);
      }
    }
    ...
    try {
      ...
      if (persistedIdentity?.primaryEntityId) {
    ...
    (async () => {
      try {
        ...
        const contextType = publishingIdentity?.context?.context_type || null;
        const articleType = publishingIdentity?.document?.article_type || publishingIdentity?.context?.article_type || null;
        const organizationName = publishingIdentity?.entities?.publishing_organization?.name || null;
        const venueName = publishingIdentity?.entities?.publication_venue?.name || null;
        const hasDistinctWebPublication = contextType === "web" && venueName && organizationName &&
          venueName.toLowerCase() !== organizationName.toLowerCase();
        const existingSourceRatings = hasDistinctWebPublication
          ? ratingRows.filter((row) => !/^(wikipedia|wikidata|scimago)$/i.test(String(row.source || "")))
          : ratingRows;
        const publicationContext = /news/i.test(articleType || "") ? "news" :
          /opinion|editorial|commentary/i.test(articleType || "") ? "opinion" :
          contextType;
        ...
        const evaluation = await evaluateAdmiraltyCode({
          ...
          publisherName: publisherLink.publisherName,
          sourceIdentity: {
            sourceType:      contextType === "web" && /news/i.test(articleType || "") ? "journalism" : dbSourceType || undefined,
