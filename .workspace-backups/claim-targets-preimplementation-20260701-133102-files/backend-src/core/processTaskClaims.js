...
export async function processTaskClaims({ query, taskContentId, text, claimType = 'task', taskClaimsContext = null, clearOldLinks = false, extractionMode = null }) {
  ...
  const result = claimIds.map((id, i) => ({
    ...
    claimKind: claimsForPersistence[i]?.claimKind || claimsForPersistence[i]?.claim_kind || null,
    evidenceType: claimsForPersistence[i]?.evidenceType || claimsForPersistence[i]?.evidence_type || null,
    extractionArticleStance: claimsForPersistence[i]?.articleStance || claimsForPersistence[i]?.article_stance || null,
    namedEntities: Array.isArray(claimsForPersistence[i]?.namedEntities) ? claimsForPersistence[i].namedEntities : [],
    dates: Array.isArray(claimsForPersistence[i]?.dates) ? claimsForPersistence[i].dates : [],
    studiesOrDocuments: Array.isArray(claimsForPersistence[i]?.studiesOrDocuments) ? claimsForPersistence[i].studiesOrDocuments : [],
    sourceCitedInArticle: claimsForPersistence[i]?.sourceCitedInArticle || "",
    isFallibilityCritical: Boolean(claimsForPersistence[i]?.isFallibilityCritical),
    whyCritical: claimsForPersistence[i]?.whyCritical || "",
    searchAssertions: Array.isArray(claimsForPersistence[i]?.searchAssertions)
      ? claimsForPersistence[i].searchAssertions
      : [],
  }));
