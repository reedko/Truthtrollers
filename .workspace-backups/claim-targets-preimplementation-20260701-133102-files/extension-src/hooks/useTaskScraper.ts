...
export const useTaskScraper = () => {
  ...
  const scrapeTask = async (passedUrl?: string) => {
    ...
    try {
      ...
      const scrapeResult = await scrapeContent(initialUrl);
      if (!scrapeResult) {
        throw new Error("Scrape did not return a content record");
      }
      console.log(
        scrapeResult.evaluationPending
          ? "⏳ Content saved; evaluation continues in the background."
          : "✅ Task and references fully scraped!",
      );
