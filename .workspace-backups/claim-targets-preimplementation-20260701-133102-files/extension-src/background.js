...
const evaluationMonitors = new Map();

async function setStoredScrapeStatus(url, status, contentId = null, error = null) {
  if (!url) return;
  const stored = await browser.storage.local.get("scrapeStatuses");
  const scrapeStatuses = { ...(stored.scrapeStatuses || {}) };
  if (status == null) {
    delete scrapeStatuses[url];
  } else {
    scrapeStatuses[url] = { status, contentId, error, updatedAt: Date.now() };
  }
  await browser.storage.local.set({ scrapeStatuses });
}

async function notifyScrapeStatus(tabId, payload) {
  try {
    ...
    // Normal web pages do not necessarily have the PDF viewer listener.
  }
}

function startEvaluationMonitor(tabId, url, contentId) {
  const token = {};
  evaluationMonitors.set(tabId, token);

  void (async () => {
    for (let attempt = 0; attempt < 600; attempt += 1) {
      if (evaluationMonitors.get(tabId) !== token) return;
      try {
        const response = await fetch(
          `${BASE_URL}/api/scrape-evaluation-status/${contentId}`,
          { method: "GET", credentials: "include" },
        );
        if (response.ok) {
          const result = await response.json();
          if (result.status === "complete" || result.status === "failed") {
            evaluationMonitors.delete(tabId);
            activeScrapes.delete(tabId);
            await setStoredScrapeStatus(url, null);
            await browser.action.setBadgeText({ tabId, text: result.status === "failed" ? "!" : "" });
            await notifyScrapeStatus(tabId, {
              url,
              status: result.status === "failed" ? "failed" : "analysis-complete",
              error: result.error || null,
            });
            // Preserve the existing display rule: only a completed human
            // evaluation auto-opens. Machine/AI results remain click-to-view.
            await checkContentAndUpdatePopup(tabId, url, false);
            return;
          }
        }
      } catch (error) {
        console.warn("⚠️ Evaluation status check failed:", error);
      }
...
browser.runtime.onMessage.addListener(async (message, sender) => {
  try {
    ...
    if (message.action === "scrapingStarted") {
      ...
      await setStoredScrapeStatus(message.url, "running");
      await browser.action.setBadgeBackgroundColor({ tabId, color: "#0ea5e9" });
      await browser.action.setBadgeText({ tabId, text: "…" });
      await hideTaskCardIfPresent(tabId);
      await notifyScrapeStatus(tabId, { url: message.url, status: "running" });

      console.log(`📊 Active scrapes: ${activeScrapes.size} tab(s) currently scraping`);
      return;
    }

    if (message.action === "scrapeEvaluationPending") {
      const tabId = sender.tab?.id ?? null;
      if (!tabId || !message.contentId) return;
      const url = message.url || activeScrapes.get(tabId)?.url;
      await setStoredScrapeStatus(url, "running", message.contentId);
      await hideTaskCardIfPresent(tabId);
      await notifyScrapeStatus(tabId, {
        url,
      ...
      startEvaluationMonitor(tabId, url, message.contentId);
      return;
    }
      ...
      try {
        if (!tabId) {
          ...
          if (tab?.url && tab.url.startsWith(viewerPrefix)) {
            ...
            try {
              // A completed scrape is not necessarily a completed evaluation.
              // Let /api/check-content decide whether automatic display is allowed.
              await checkContentAndUpdatePopup(tabId, url, false);
...
async function checkContentAndUpdatePopup(tabId, url, forceVisible) {
  ...
  try {
    ...
    // Automatic display is allowed only for a completed evaluation. The only
    // bypass is an explicit extension-icon click (`forceVisible=true`). Merely
    // existing in the database, having an AI estimate, or being eligible for
    // "Add to TruthTrollers" must not open the popup by itself.
    if (isCompleted || forceVisible) {
      await showTaskCard(tabId, isCompleted, forceVisible);
    } else {
      await hideTaskCardIfPresent(tabId);
    }
  } catch (error) {
  ...
  try {
    ...
    const [{ result: popupFullyRendered }] = await browser.scripting.executeScript({
      ...
      func: () => {
        ...
        const popupRoot = popupHost?.shadowRoot?.getElementById("tt-popup-root");
...
async function hideTaskCardIfPresent(tabId) {
  try {
    await browser.scripting.executeScript({
      target: { tabId },
      func: () => {
    ...
    console.warn("⚠️ Could not hide incomplete-evaluation popup:", err);
  }
}
