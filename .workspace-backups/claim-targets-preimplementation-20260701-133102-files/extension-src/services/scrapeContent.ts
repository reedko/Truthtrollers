...
export type ScrapeContentResult = {
  contentId: string;
  evaluationPending: boolean;
};
 ...
 */
export async function scrapeContent(url: string): Promise<ScrapeContentResult | null> {
  ...
  if (isFacebookPost(normUrl)) {
    ...
    const contentId = await scrapeFacebookPost(true); // createContent = true
    return contentId ? { contentId, evaluationPending: false } : null;
  }
  ...
  let payload: any = {
    ...
    // "Add" should finish once the case and its metadata are persisted. The
    // backend can continue claim extraction/evidence evaluation independently.
  ...
  console.log(
    backendResp.evaluationPending
      ? `⏳ Task persisted; evaluation running: content_id=${contentId}`
      : `✅ Task complete: content_id=${contentId}`,
  );
  ...
  return {
    contentId,
    evaluationPending: backendResp.evaluationPending === true,
  };
}
