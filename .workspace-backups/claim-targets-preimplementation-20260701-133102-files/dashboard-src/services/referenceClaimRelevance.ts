...
export async function fetchReferenceClaimTaskLinks(
  ...
  const links = (await response.json()) as ReferenceClaimTaskLink[];
  return links.map((link) => ({
    ...
    support_level: Number(link.support_level) || 0,
    reference_content_id: link.reference_content_id == null
      ? undefined
      : Number(link.reference_content_id),
  }));
}
