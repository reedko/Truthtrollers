  ...
  return (
    <Box
      ...
      border={isActive
        ...
        : isUser
          ? `2px solid ${color}`
          : `1px ${data.provenance === "ai" ? "dashed" : "solid"} ${color}99`}
      borderRadius={
        data.kind === "content"
          ? "full"
      ...
      boxShadow={`0 10px 28px rgba(0,0,0,0.45), 0 0 ${isActive ? 34 : isFocus || isUser ? 24 : 10}px ${color}${isActive ? "aa" : "44"}, ${isActive ? `0 0 0 5px ${color}22, ` : ""}inset 0 1px 0 rgba(255,255,255,0.08)`}
      overflow="hidden"
      color="var(--mr-text-primary)"
      transform={data.exiting ? "scale(0.78)" : data.outOfFocus ? "scale(0.72)" : isActive ? "scale(1.08)" : data.focusSecondHop ? "scale(0.9)" : "scale(1)"}
        ...
        {data.kind === "source" && data.reference ? (
          <SourceCrest
            {...normalizeSourceProfile({
              ...
              source_type: data.reference.source_type ?? undefined,
...
function EvidenceMapFlow({
  ...
  useEffect(() => {
    let active = true;
    const focusedGraph = annotateFocusNeighborhood(flow.nodes, flow.edges, flow.adjacency, focusNodeId, activeNodeId);
    setIsLayouting(true);
  ...
  }, [flow, focusNodeId, activeNodeId, setNodes, setEdges, fitView, setCenter]);
  ...
  const handleNodeClick = useCallback(
    (_: React.MouseEvent, node: Node<EvidenceNodeData>) => {
      ...
      // A source/evidence claim belongs in the blossom of its case claim.
      // Keep the clicked source claim active, but center and expand its linked
      // case claim so sibling source claims are visible in the same group.
      const linkedCaseClaimId = node.data.kind === "evidenceClaim"
        ? Array.from(flow.adjacency.get(node.id) || []).find((neighborId) =>
            flow.nodes.find((candidate) => candidate.id === neighborId)?.data.kind === "taskClaim",
          )
        : undefined;

      setFocusNodeId(linkedCaseClaimId || node.id);
      setActiveNodeId(node.id);
      setSelectedNodeData(node.data);
    },
    [flow.adjacency, flow.nodes],
  );
              ...
              <HStack spacing={3} minW={0} flex="1">
                <SourceCrest
                  {...normalizeSourceProfile({
                    ...
                    source_type: selectedNodeData.reference.source_type ?? undefined,
