...
const systemPrompt = String.raw`You extract a reasoning stack from content.
...
const withTopicsPrompt = String.raw`Analyze the article text and return strict JSON using exactly the structure below.
...
async function main() {
  ...
  try {
    ...
    const [systemResult] = await conn.execute(
      "UPDATE llm_prompts SET prompt_text = ?, version = version + 1 WHERE prompt_name = 'claim_extraction_stack_system' AND is_active = 1",
      [systemPrompt]
    );
    const [withTopicsResult] = await conn.execute(
      "UPDATE llm_prompts SET prompt_text = ?, version = version + 1 WHERE prompt_name = 'claim_extraction_stack_with_topics' AND is_active = 1",
      [withTopicsPrompt]
    );

    console.log(JSON.stringify({
      content_claims_enum: "updated",
      claim_extraction_stack_system_rows: systemResult.affectedRows,
      claim_extraction_stack_with_topics_rows: withTopicsResult.affectedRows,
    }, null, 2));
  } finally {
    await conn.end();
  }
