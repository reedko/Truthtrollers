-- Fix delete_content_cascade using JOINs with correct column names
...
BEGIN
    ...
    -- Step 5: Delete user_claim_ratings (task_claim_id) using JOIN
    DELETE ucr FROM user_claim_ratings ucr
    INNER JOIN content_claims cc ON ucr.task_claim_id = cc.claim_id
    WHERE cc.content_id = content_id_to_delete;

    -- Step 6: Delete user_claim_ratings (reference_claim_id) using JOIN
    DELETE ucr FROM user_claim_ratings ucr
    INNER JOIN content_claims cc ON ucr.reference_claim_id = cc.claim_id
    WHERE cc.content_id = content_id_to_delete;

    -- Step 7: Delete orphaned claims (claims only in this content)
...
SELECT '✅ Fixed: using correct column names for user_claim_ratings' AS status;
