-- Fix delete_content_cascade using JOINs instead of subqueries
-- This avoids all the MySQL subquery issues
...
BEGIN
    ...
    -- Step 1: Delete reference_claim_task_links using JOIN
    ...
    -- Step 5: Delete user_claim_ratings using JOIN
    DELETE ucr FROM user_claim_ratings ucr
    INNER JOIN content_claims cc ON ucr.claim_id = cc.claim_id
    WHERE cc.content_id = content_id_to_delete;

    -- Step 6: Delete orphaned claims (claims only in this content)
    ...
    -- Step 12: Delete author/publisher relations
    ...
    -- Step 13: Delete task completions
...
SELECT '✅ Stored procedure fixed using JOINs' AS status;
