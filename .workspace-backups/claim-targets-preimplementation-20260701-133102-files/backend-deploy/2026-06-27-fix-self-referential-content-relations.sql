-- Fix: prevent self-referential content_relations in stored procedure
-- and clean up existing bad records created by SourceDetailModal re-scrape bug.
-- Root cause: SourceDetailModal passed taskContentId = contentId (the reference's
-- own ID) when triggering a publisher re-scrape, causing content_id = reference_content_id.

USE truthtrollers;

-- 1. Show how many bad records exist before cleanup
SELECT COUNT(*) AS self_referential_records_to_delete
FROM content_relations
WHERE content_id = reference_content_id;

-- 2. Delete all self-referential records
DELETE FROM content_relations
WHERE content_id = reference_content_id;

SELECT ROW_COUNT() AS deleted_self_referential_records;

-- 3. Update stored procedure to guard against self-referential inserts
...
BEGIN
    ...
    -- Guard: only link if taskContentId is set AND different from contentId (no self-references)
    IF contentType = 'reference' AND taskContentId IS NOT NULL AND taskContentId != contentId THEN
...
SELECT 'InsertContentAndTopics updated: self-referential guard added' AS Status;
