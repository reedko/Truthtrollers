...
import { buildEvidenceNeedV1 } from "./evidenceNeed.js";
...
export async function runEvidenceEngine({
  ...
  const claims = claimIds
    ...
    .map((row) => {
      ...
      if (isBearingShadowEnabled() || bearingConfig.enableBearingGating) {
  ...
  const runOptions = {
    ...((isBearingShadowEnabled() || bearingConfig.enableBearingGating) ? { taskContentId } : {}),
